...
#[proc_macro_derive(ShowSize)]
pub fn show_size(tokens: TokenStream) -> TokenStream {
    ...
    let query_parts = show_name(&fields_punct);

    let modified = quote! {
        impl<'a> #name<'a> {
            pub fn show_size(&self) -> String {
...
fn show_name(
    ...
    let fields = fields.iter().enumerate().map(move |(i, field)| {
        ...
        let ty = match &field.ty {
            Type::Path(t) => t.path.segments.first().unwrap().ident.to_string(),
            _ => "".to_string(),
        };
